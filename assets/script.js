...
function setupEventListeners() {
  ...
  // Pagination
  elements.pagination.firstPage.addEventListener("click", () =>
    navigateToPage(1)
  );
  elements.pagination.prevPage.addEventListener("click", () =>
    navigateToPage(currentPage - 1)
  );
  elements.pagination.nextPage.addEventListener("click", () =>
    navigateToPage(currentPage + 1)
  );
  elements.pagination.lastPage.addEventListener("click", () =>
...
function displayEmails() {
  ...
  if (searchQuery) {
    filteredEmails = emailsData.filter(
      (email) =>
        (email.raw_emailid &&
          email.raw_emailid.toLowerCase().includes(searchQuery)) ||
        (email.sp_account &&
          email.sp_account.toLowerCase().includes(searchQuery)) ||
    ...
    rowElement.innerHTML = `
            ...
            <td class="px-6 py-4 text-sm text-gray-500">
                <div class="tooltip">
                    ...
                    ${
                      row.validation_response
                        ? `<span class="tooltiptext">${row.validation_response}</span>`
                        : ""
                    }
                </div>
            </td>
...
function showStatusMessage(message, type = "info") {
  ...
  elements.statusMessage.className = `mt-4 text-center p-3 rounded-md ${
