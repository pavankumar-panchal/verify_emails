...
<body class="bg-gray-100">
    <div class="main-content container mx-auto px-4 py-8 max-w-7xl">
        <!-- Header -->
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 pb-6 border-b border-gray-200">
            <div>
            ...
            <button onclick="window.location.reload()" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md transition">
        ...
        <div class="mb-6">
            <div class="flex overflow-x-auto pb-2 gap-2">
                <?php foreach ($smtps as $index => $smtp): ?>
                    <button onclick="switchAccount(<?= $index ?>)" class="account-tab whitespace-nowrap px-4 py-2 rounded-md border border-gray-200 hover:bg-gray-50 transition <?= $index === 0 ? 'bg-blue-600 text-white border-blue-600 hover:bg-blue-700' : 'bg-white' ?>">
        ...
        <?php foreach ($smtps as $index => $smtp): 
            ...
            <div class="account-content <?= $index === 0 ? 'block' : 'hidden' ?>" id="account-<?= $index ?>">
                <!-- Account Info -->
                <div class="flex flex-col md:flex-row gap-4 items-start md:items-center mb-6 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                    <div class="flex-1">
                ...
                <?php else: ?>
                    ...
                    <div class="space-y-8">
                        ...
                        <?php if (!empty($replies['regular'])): ?>
                            <div>
                                ...
                                <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200">
                                    <!-- Email Header -->
                                    <div class="grid grid-cols-12 gap-4 p-4 bg-gray-50 border-b border-gray-200 font-medium text-gray-600">
                                    ...
                                    <?php foreach ($replies['regular'] as $reply): ?>
                                        <!-- Email Item -->
                                        <div class="email-item grid grid-cols-12 gap-4 p-4 border-b border-gray-200 hover:bg-gray-50 transition cursor-pointer <?= !$reply['seen'] ? 'bg-blue-50' : '' ?>" onclick="toggleEmailBody(this, <?= $index ?>, '<?= $reply['uid'] ?>')">
                                            ...
                                            <div class="col-span-3 md:col-span-2 flex items-center gap-2 overflow-hidden">
                                                ...
                                                <span class="truncate" title="<?= htmlspecialchars($reply['from'] ?: $reply['from_email']) ?>"><?= htmlspecialchars($reply['from'] ?: $reply['from_email']) ?></span>
                                            </div>
                                        ...
                                        <div class="email-body hidden" id="email-body-<?= $index ?>-<?= $reply['uid'] ?>">
                                            <div class="p-6 border-b border-gray-200">
                                                <div class="flex flex-col md:flex-row justify-between gap-4 mb-4">
                                                    <div>
                                                        <h3 class="text-lg font-bold mb-2"><?= htmlspecialchars($reply['subject']) ?></h3>
                                                        <p class="text-gray-700 mb-1">
                                                            <strong class="text-gray-600">From:</strong>
                                                            <?= htmlspecialchars($reply['from'] ? $reply['from'] . ' &lt;' . $reply['from_email'] . '&gt;' : $reply['from_email']) ?>
                                                        </p>
                                                        <p class="text-gray-700">
                                                            <strong class="text-gray-600">Date:</strong> <?= formatDate($reply['date'], true) ?>
                                                        </p>
                                                    </div>
                                                    <div class="flex gap-2">
                                                        <button class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md transition">
                        ...
                        <?php if (!empty($replies['unsubscribes'])): ?>
                            <div>
                                <h3 class="text-lg font-semibold mb-4 text-gray-700 flex items-center gap-2">
                                    <i class="fas fa-user-minus text-red-500"></i> Unsubscribe Requests (<?= count($replies['unsubscribes']) ?>)
                                </h3>
                                <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200 border-l-4 border-l-red-500">
                                    <!-- Email Header -->
                                    <div class="grid grid-cols-12 gap-4 p-4 bg-red-50 border-b border-gray-200 font-medium text-gray-600">
                                        <div class="col-span-1"></div>
                                        <div class="col-span-3 md:col-span-2">Sender</div>
                                        <div class="col-span-6 md:col-span-7">Subject</div>
                                        <div class="col-span-2 md:col-span-2 text-right">Date</div>
                                    ...
                                    <?php foreach ($replies['unsubscribes'] as $reply): ?>
                                        <div class="email-item grid grid-cols-12 gap-4 p-4 border-b border-gray-200 hover:bg-gray-50 transition cursor-pointer bg-red-50" onclick="toggleEmailBody(this, <?= $index ?>, '<?= $reply['uid'] ?>')">
                                            <div class="col-span-1 flex items-center">
                                                <input type="checkbox" class="rounded text-blue-600 focus:ring-blue-500">
                                            </div>
                                            <div class="col-span-3 md:col-span-2 flex items-center gap-2 overflow-hidden">
                                            ...
                                            <div class="p-6 border-b border-gray-200">
                                                <div class="flex flex-col md:flex-row justify-between gap-4 mb-4">
                                                    ...
                                                    <div class="flex gap-2">
                                                        ...
                                                        <button onclick="event.stopPropagation(); processUnsubscribe('<?= $reply['from_email'] ?>', <?= $index ?>, '<?= $reply['uid'] ?>')" class="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md transition">
                        ...
                        <?php if (!empty($replies['bounces'])): ?>
                            <div>
                                <h3 class="text-lg font-semibold mb-4 text-gray-700 flex items-center gap-2">
                                    <i class="fas fa-exclamation-triangle text-yellow-500"></i> Bounced Emails (<?= count($replies['bounces']) ?>)
                                </h3>
                                <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200 border-l-4 border-l-yellow-500">
                                    <!-- Email Header -->
                                    <div class="grid grid-cols-12 gap-4 p-4 bg-yellow-50 border-b border-gray-200 font-medium text-gray-600">
                                        <div class="col-span-1"></div>
                                        <div class="col-span-3 md:col-span-2">Sender</div>
                                        <div class="col-span-6 md:col-span-7">Subject</div>
                                        <div class="col-span-2 md:col-span-2 text-right">Date</div>
                                    ...
                                    <?php foreach ($replies['bounces'] as $reply): ?>
                                        <div class="email-item grid grid-cols-12 gap-4 p-4 border-b border-gray-200 hover:bg-gray-50 transition cursor-pointer bg-yellow-50" onclick="toggleEmailBody(this, <?= $index ?>, '<?= $reply['uid'] ?>')">
                                            <div class="col-span-1 flex items-center">
                                                <input type="checkbox" class="rounded text-blue-600 focus:ring-blue-500">
                                            </div>
                                            <div class="col-span-3 md:col-span-2 flex items-center gap-2 overflow-hidden">
                                        ...
                                        <div class="email-body hidden" id="email-body-<?= $index ?>-<?= $reply['uid'] ?>">
                                            <div class="p-6 border-b border-gray-200">
                                                <div class="flex flex-col md:flex-row justify-between gap-4 mb-4">
                                                    ...
                                                    <div class="flex gap-2">
                                                        <button onclick="event.stopPropagation(); processBounce('<?= $reply['from_email'] ?>', <?= $index ?>, '<?= $reply['uid'] ?>')" class="flex items-center gap-2 bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-2 rounded-md transition">
    ...
    <div id="progressOverlay" class="progress-overlay hidden">
        <div class="circle-loader">
            <svg width="180" height="180">
                ...
                <circle class="circle-progress" cx="90" cy="90" r="75" stroke-dasharray="471" stroke-dashoffset="471"></circle>
            </svg>
